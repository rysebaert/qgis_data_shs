...
Pour **modéliser la réalité**, on superpose plusieurs couches d'information géographique,  **vecteur et/ou raster**, de différente nature, dans un même système de projection
...
**La Terre n'est pas une sphère** qui est une forme géométrie dite parfaite. **Elle est légèrement aplatie aux pôles et bosselée selon les continents**. Son apparence sphérique lorsqu'on l'observe depuis l'espace masque les nombreuses petites irrégularités de sa surface. On ne peut lui appliquer des **relations mathématiques**.
...
Il sert à déterminer les coordonnées géographiques d'un on objet sur la surface de la Terre > une mesure d'**angles** au regard d'un parallèle (équateur) et un méridien (Greenwich) de référence. Ces coordonnées traduisent au minumum deux dimensions : la latitude et la longitude.
...
```{r, echo=FALSE,out.width="48%", out.height="20%",fig.show='hold',fig.align='center'}
...
« ***La graphique*** *utilise les propriétés de l'image visuelle pour faire apparaître les* ***relations*** *de* ***différence***, ***d'ordre*** et de ***proportionnalité*** entre les données » : rendu possible grâce des variables visuelles (ou rétiniennes).
]
